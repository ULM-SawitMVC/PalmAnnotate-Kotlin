...
- **[Releases](../../releases)**: APK versi rilis, link permanen. Untuk koleksi lapangan,
  pilih `PalmAnnotate-field-*.apk`. Aset `PalmAnnotate-debug-*.apk` adalah aplikasi diagnostik
  berdampingan (`applicationId` = `dev.sawitulm.palmannotate.trace`) yang **debuggable** dan
  terukur lebih lambat; jangan dipakai mengambil dataset.
